...
public class MemberController {
	...
	private EcoDto showJoinView() {
		...
		while (!isFinish) {
			...
			if (phone.equals(uniqueCheckPhone(phone))) {
				System.out.println("중복 된 핸드폰이가 있습니다.");
				System.out.println("재시도 하시겠습니까 ? ( Y / N )");
				String user = InputUtil.toUpperCase(InputUtil.getString());
				isFinish = request(user);
				if (user.equals("N")) {
	...
	public boolean request(String user) { // 재시도 메시지 날림~~ 반복문 실행시 !ISFINSH 여야 반복문 진행 되니까 FALSE~로 리턴~
		boolean isFinish = false;
	...
	public String uniqueCheck(String id) { // 쿼리문으로 중복검사할거임 입력한 아이디가있다면 DB 아이디 불러옴~
		Connection conn = MiniConn.getConnection();
		PreparedStatement pstmt = null;
	...
	public String uniqueCheckPhone(String phone) { // 쿼리문으로 중복검사할거임~~ 입력한 아이디가있다면 DB 아이디 불러옴~
		Connection conn = MiniConn.getConnection();
		PreparedStatement pstmt = null;
